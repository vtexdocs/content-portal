...
const docsPathsGLOBAL = await getTracksPaths('documentation')
...
export const getStaticPaths: GetStaticPaths = async () => {
  const slugs: { [slug: string]: { locale: string; path: string }[] } =
    await getTracksPaths('documentation')
...
export const getStaticProps: GetStaticProps = async ({
  ...
  const docsPaths =
    process.env.NEXT_PHASE === PHASE_PRODUCTION_BUILD
      ? docsPathsGLOBAL
      : await getTracksPaths('documentation', branch)
  ...
  try {
    ...
    parentsArrayName.forEach((_el: string, idx: number) => {
      breadcrumbList.push({
        slug: `/docs/documentation/${parentsArray[idx]}`,
